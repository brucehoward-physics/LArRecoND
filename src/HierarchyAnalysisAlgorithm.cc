...
StatusCode HierarchyAnalysisAlgorithm::Run()
{
    ...
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, PandoraContentApi::GetList(*this, m_caloHitListName3D, pCaloHitList3D));
    ...
    // BH! BH! BH! TEST:
    for ( CaloHitList::const_iterator itCaloHit3D=pCaloHitList3D->begin(); itCaloHit3D != pCaloHitList3D->end(); ++itCaloHit3D ){ 
        const pandora::CaloHit *pCaloHit3D = *itCaloHit3D;
...
void HierarchyAnalysisAlgorithm::EventAnalysisOutput(const LArHierarchyHelper::MatchInfo &matchInfo) const
{
    ...
    // and trying to get the calorimetry info
    ...
    for (const ParticleFlowObject *const pRoot : rootPfos)
    {
        ...
        for (const LArHierarchyHelper::RecoHierarchy::Node *pRecoNode : recoNodes)
        {
            ...
            for (const ParticleFlowObject *pPfo : recoParticles)
            {
		        ...
		        // Let's also try the sliding track fit for our Pfo
                ...
                if (!trackStateSuccess || trackStateVector.size() < m_minTrajectoryPoints) {
                    // Not using fit, so fill some default values
...
StatusCode HierarchyAnalysisAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
{
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=, XmlHelper::ReadValue(xmlHandle, "CaloHitListName3D", m_caloHitListName3D));
